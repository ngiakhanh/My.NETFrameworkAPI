...
namespace My.NETWebAPI.Handlers
{
    public class ForwardedHeadersHandler : DelegatingHandler
    {
        ...
        const string _fwdForHeader = "X-Forwarded-For";
        /// <summary>
        ...
        public const string MyClientIp = "MyClientIp";
              ...
              HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            {
                client = request.Headers.GetValues(_fwdForHeader)
                        .FirstOrDefault(s => !String.IsNullOrEmpty(s));
                if (client != null)
                    request.Properties.Add(MyClientIp, client);            
            }
            ...
            else if (request.Headers.Contains(_fwdHeader))
            {
                ...
                client = fwd.FirstOrDefault(s => s.ToLowerInvariant().StartsWith("for="));
                if (!String.IsNullOrEmpty(client))
                {
                    client = client.Substring(4).Trim();
                    if (!String.IsNullOrEmpty(client))
                        request.Properties.Add(MyClientIp, client);
                }
            }
            ...
            else
            {
                if (request.Properties.ContainsKey("MS_HttpContext"))
                {
                    ...
                    if (ctx != null)
                        client = ctx.Request.UserHostAddress;
    ...
    public static class HttpRequestMessageBaseUrlExtension
    {
        ...
        public static string GetClientIpAddress(this HttpRequestMessage request)
        {
            ...
            if (request.Properties.TryGetValue(ForwardedHeadersHandler.MyClientIp,
                out var clientIp))
            {
                return clientIp.ToString().Split(',').Select(s => s.Trim()).FirstOrDefault(s => !String.IsNullOrEmpty(s));
            }
